...
// Agregamos mostrarDorso = false como prop para que el SuperAdmin pueda pedir la parte de atrás
const CarnetJugadora = ({ jugadora, config, mostrarDorso = false }) => {
  ...
  // --- LÓGICA DE CATEGORÍA DINÁMICA (CON FIX DE ZONA HORARIA) ---
  useEffect(() => {
    const calcularCategoriaSaaS = async () => {
      if (!jugadora) return;

      // REGLA 1: LIGA DE LAS NENAS (Cálculo matemático puro)
      if (jugadora.organizacion_id === ID_LIGA_NENAS && jugadora.fecha_nacimiento) {
        // IMPORTANTE: getUTCFullYear evita que el 01/01/2011 se convierta en 2010
        const fecha = new Date(jugadora.fecha_nacimiento);
        const añoNac = fecha.getUTCFullYear();
        
        if (añoNac >= 1994 && añoNac <= 2008) {
          setNombreCategoria("Primera (Libre)");
        } else if (añoNac < 1994) {
          setNombreCategoria("Veteranas");
        } else {
          // Fórmula para bienios (2011-2012, etc.)
          const inicioRango = añoNac % 2 === 0 ? añoNac - 1 : añoNac;
          setNombreCategoria(`${inicioRango}-${inicioRango + 1}`);
        }
        return;
      }

      // REGLA 2: OTRAS LIGAS (Búsqueda en tabla categorías)
  ...
  const handleDescargarPDF = async () => {
    ...
    // Para el PDF individual, forzamos un render temporal del dorso si es necesario
    ...
    <div className="flex flex-col items-center mt-6 space-y-6">
      <div className="flex flex-col gap-6 scale-110">
        ...
        <div 
          ...
          className="rounded-xl p-3 shadow-2xl relative overflow-hidden border border-white/10 carnet-container-pdf"
        >
          {!mostrarDorso ? (
            /* CONTENIDO FRONTAL */
            <>
              ...
              <div className="z-10 flex justify-between items-start">
                ...
                {(jugadora.club_escudo || jugadora.equipos?.logo_url || jugadora.equipos?.escudo_url) && (
                  <img 
                    ...
                    className="h-10 w-12 object-contain bg-white/10 rounded-md p-0.5" 
                    alt="club"
            ...
            /* CONTENIDO DEL DORSO (QR) */
            <div className="z-10 w-full h-full flex items-center justify-between px-4" ref={dorsoRef}>
               <div className="flex flex-col items-center w-1/2">
                  <div className="w-28 h-24 bg-white/5 border border-white/10 rounded-full flex items-center justify-center p-4">
                    {config?.logo_url && (
                      <img 
                        src={config.logo_url} 
