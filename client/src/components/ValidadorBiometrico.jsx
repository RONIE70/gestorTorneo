...
// --- 1. PARCHE MAESTRO ANTI-CRASH (VITE/EXIF-JS) ---
// Este bloque soluciona el error "n is not defined" que ves en la consola.
// El error viene de un bug interno de exif-js (visto en image_50ac66.png).
if (typeof window !== 'undefined') {
    // Definimos 'n' globalmente para que el bucle de la librería no falle.
    window.n = window.n || 0; 

    // Parche para face-api.js en entornos minificados.
...
const analizarForense = (url) => {
 ...
 const ejecutarCheckCompleto = async (jugadora) => {
   ...
   const actualizarEstado = async (id, nuevoEstado, distancia) => {
    ...
    if (cargandoModelos) return <div className="p-20 text-center text-white font-black animate-pulse uppercase tracking-[0.3em]">Cargando Cerebro IA...</div>;
